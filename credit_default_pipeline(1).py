...
X, X_test = _feat['X'], _feat['X_test']
y = _feat['y'] if 'y' in _feat else target.set_index(CFG.ID_COL).reindex(X.index)[CFG.TARGET].astype(int)
importances = _feat.get('importances')
print(f"Удалено: константных {len(_feat.get('dropped_const', []))}, коррелированных {len(_feat.get('dropped_corr', []))}")
...
def run_cv_cached(name, fit_predict_fn):
    try:
        ...
        r = RESULTS[name]
        print(f"{name:16s} OOF AUC={r['auc']:.5f}  folds={np.round(r['fold_aucs'], 4).tolist()}  time={r['time']:.1f}s")
        return r
    except Exception as e:
        print(f"[ПРОПУСК] {name}: {type(e).__name__}: {e}")
        return None
...
if not any(c.endswith('_te') for c in X.columns):
    try:
        def add_oof_te(X, X_test, y, folds, cols, smoothing=20):
            prior = y.mean(); yv = y.values; tr_new, te_new = {}, {}
            for c in cols:
                oof = np.zeros(len(X))
                for tr, va in folds:
                    d = pd.DataFrame({'c': X[c].values[tr], 'y': yv[tr]})
                    agg = d.groupby('c')['y'].agg(['mean', 'count'])
                    enc = (agg['mean'] * agg['count'] + prior * smoothing) / (agg['count'] + smoothing)
                    oof[va] = pd.Series(X[c].values[va]).map(enc).fillna(prior).values
                tr_new[f'{c}_te'] = oof.astype(np.float32)
                d = pd.DataFrame({'c': X[c].values, 'y': yv})
                agg = d.groupby('c')['y'].agg(['mean', 'count'])
                enc = (agg['mean'] * agg['count'] + prior * smoothing) / (agg['count'] + smoothing)
                te_new[f'{c}_te'] = X_test[c].map(enc).fillna(prior).values.astype(np.float32)
            return tr_new, te_new
        te_cols = [c for c in X.columns if c.endswith('_last') and X[c].nunique() <= 30]
        tr_te, te_te = add_oof_te(X, X_test, y, folds, te_cols)
        for c, arr in tr_te.items():  X[c] = arr
        for c, arr in te_te.items():  X_test[c] = arr
        del tr_te, te_te; gc.collect()
        tmp = apath('features.joblib') + '.tmp'
        joblib.dump({'X': X, 'X_test': X_test}, tmp); os.replace(tmp, apath('features.joblib'))
        print(f'Добавлено TE-признаков: {len(te_cols)} | X: {X.shape}')
    except Exception as e:
        print(f'[ПРОПУСК] target encoding: {type(e).__name__}: {e}')
else:
    print('TE-признаки уже на месте.')
...
if not any(c.endswith('_lag1') for c in X.columns):
    try:
        globals().pop('_feat', None); globals().pop('_f', None)
        for _df in (X, X_test):
            for c in _df.columns:
                dt = _df[c].dtype
                if dt == np.float64:   _df[c] = _df[c].astype(np.float32)
                elif dt == np.int64:   _df[c] = _df[c].astype(np.int32)
        gc.collect()
        print(f'После понижения типов свободно: {_mem_avail_gb():.1f} ГБ')

        import pyarrow.parquet as pq
        seq_feats = groups['all']
        def _read_lag(parquet_path, index_ref):
            pf = pq.ParquetFile(path(parquet_path))
            cols = [CFG.ID_COL, CFG.SEQ_COL] + seq_feats
            il, rl, fl = [], [], []
            for b in pf.iter_batches(batch_size=500_000, columns=cols):
                d = b.to_pandas()
                il.append(d[CFG.ID_COL].to_numpy())
                rl.append(d[CFG.SEQ_COL].to_numpy().astype(np.int32))
                fl.append(d[seq_feats].to_numpy().astype(np.int8))
                del d
            idv = np.concatenate(il); rnv = np.concatenate(rl); fv = np.concatenate(fl)
            del il, rl, fl; gc.collect()
            order = np.lexsort((rnv, idv))
            ids_sorted = idv[order]; del idv, rnv; gc.collect()
            uniq, first = np.unique(ids_sorted, return_index=True)
            offsets = np.append(first, len(ids_sorted)); del ids_sorted; gc.collect()
            has_prev = (offsets[1:] - offsets[:-1]) >= 2
            last_pos = order[offsets[1:] - 1]; prev_pos = order[np.maximum(offsets[1:] - 2, 0)]
            del order; gc.collect()
            lag = np.where(has_prev[:, None],
                           fv[last_pos].astype(np.int16) - fv[prev_pos].astype(np.int16), 0).astype(np.int16)
            del fv; gc.collect()
            df = pd.DataFrame(lag, index=uniq, columns=[f'{c}_lag1' for c in seq_feats])
            return df.reindex(index_ref).fillna(0).astype(np.int16)

        lag_tr = _read_lag(CFG.TRAIN_DATA, X.index)
        lag_te = _read_lag(CFG.TEST_DATA, X_test.index)
        print(f'Лаги посчитаны. Свободно: {_mem_avail_gb():.1f} ГБ')

        for c in lag_tr.columns:  X[c] = lag_tr[c].values
        for c in lag_te.columns:  X_test[c] = lag_te[c].values
        del lag_tr, lag_te; gc.collect()

        _save = {'X': X, 'X_test': X_test}
        for _k in ('y', 'importances'):
            if _k in globals(): _save[_k] = globals()[_k]
        tmp = apath('features.joblib') + '.tmp'
        joblib.dump(_save, tmp, compress=3); os.replace(tmp, apath('features.joblib')); del _save; gc.collect()
        print(f'Готово. Лаг-признаки добавлены: {X.shape} | свободно: {_mem_avail_gb():.1f} ГБ')
    except Exception as e:
        print(f'[ПРОПУСК] лаг-признаки: {type(e).__name__}: {e}')
else:
    print('Лаг-признаки уже на месте.')
...
if HAS_LGB:
    def fit_lgb_dart(Xtr, ytr, Xva, yva, Xte):
        m = lgb.LGBMClassifier(boosting_type='dart', n_estimators=1500, learning_rate=0.03,
        ...
        m.fit(Xtr, ytr)
        return m.predict_proba(Xva)[:, 1], m.predict_proba(Xte)[:, 1]
    run_cv_cached('LightGBM_DART', fit_lgb_dart)
else:
    print('LightGBM недоступен — DART пропущен.')
...
if CFG.RUN_NN and HAS_TORCH:
    ...
    for enc in ['gru']:
        ...
        try:
            ...
            print(f'[ПРОПУСК] NN-{enc}: {type(e).__name__}: {e}')
else:
...
print(comp.style.format({'auc': '{:.5f}', 'fold0_auc': '{:.5f}', 'time_s': '{:.1f}'}))
...
yv = y.values
all_models = [k for k, v in RESULTS.items() if 'oof' in v]

def stack_auc(models):
    if len(models) == 1:
        return roc_auc_score(yv, RESULTS[models[0]]['oof'])
    OOF = np.column_stack([RESULTS[k]['oof'] for k in models])
    p = cross_val_predict(LogisticRegression(C=1.0, max_iter=2000), OOF, yv,
                          cv=5, method='predict_proba')[:, 1]
    return roc_auc_score(yv, p)

full = stack_auc(all_models)
keep = all_models[:]; best = full
while len(keep) > 2:
    score, drop = max((stack_auc([m for m in keep if m != k]), k) for k in keep)
    if score > best + 1e-5:
        keep.remove(drop); best = score; print(f'  убрали {drop} -> стек {best:.5f}')
    else:
        break
stack_models = keep
print('\nВ стеке:', stack_models)
for k in stack_models: print(f'  {k:18s} OOF AUC={RESULTS[k]["auc"]:.5f}')
...
print(f'СТЕК (честный CV)    = {auc_stack:.5f}  (полный был {full:.5f})')
...
if CFG.RUN_NN and HAS_TORCH and 'final_test' in globals():
    globals().pop('X', None); globals().pop('X_test', None); globals().pop('_feat', None); gc.collect()
    PL_SEEDS = [42]
    PL_MAX_TRAIN = 1_000_000
    PL_EPOCHS = getattr(CFG, 'NN_EPOCHS', 8)

    yv = y.values.astype(np.float32)
    pseudo = np.asarray(final_test, dtype=np.float32)
    n_tr, n_te = len(tr_off) - 1, len(te_off) - 1
    assert len(pseudo) == n_te, 'размер final_test != числу тестовых id'
    comb_flat = np.concatenate([tr_flat, te_flat])
    comb_off  = np.concatenate([tr_off, te_off[1:] + tr_off[-1]])
    comb_lab  = np.concatenate([yv, pseudo])
    test_ids_comb = np.arange(n_tr, n_tr + n_te)
    print(f'comb_flat: {comb_flat.shape} | train id: {n_tr} | test id: {n_te}')

    def train_nn_pl(tr_real, va_idx):
        seed_everything()
        if len(tr_real) > PL_MAX_TRAIN:
            tr_real = np.random.default_rng(CFG.SEED).choice(tr_real, size=PL_MAX_TRAIN, replace=False)
        ytr = yv[tr_real]; pos_w = (len(ytr) - ytr.sum()) / max(ytr.sum(), 1)
        tr_idx = np.concatenate([tr_real, test_ids_comb])
        rank_w = getattr(CFG, 'NN_RANK_W', 1.0); nw = getattr(CFG, 'NN_WORKERS', 0)
        tl  = DataLoader(SeqDataset(comb_flat, comb_off, idx=tr_idx, targets=comb_lab),
                         batch_size=CFG.NN_BATCH, shuffle=True,  collate_fn=collate, num_workers=nw)
        vl  = DataLoader(SeqDataset(comb_flat, comb_off, idx=va_idx, targets=comb_lab),
                         batch_size=CFG.NN_BATCH, shuffle=False, collate_fn=collate, num_workers=nw)
        tel = DataLoader(SeqDataset(te_flat, te_off),
                         batch_size=CFG.NN_BATCH, shuffle=False, collate_fn=collate, num_workers=nw)
        model = CreditSeqNet(vocab_sizes, encoder='gru', hidden=CFG.NN_HIDDEN).to(DEVICE)
        opt = torch.optim.AdamW(model.parameters(), lr=CFG.NN_LR, weight_decay=1e-5)
        crit = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([pos_w], device=DEVICE))
        yva = comb_lab[va_idx]; best_auc, best_state = -1, None
        for ep in range(PL_EPOCHS):
            model.train()
            for xb, lb, yb in tl:
                xb, lb, yb = xb.to(DEVICE), lb.to(DEVICE), yb.to(DEVICE)
                opt.zero_grad()
            ...
            if auc > best_auc:
                best_auc = auc; best_state = {k: v.cpu().clone() for k, v in model.state_dict().items()}
            print(f'    эпоха {ep+1}/{PL_EPOCHS}  val AUC={auc:.4f}')
        model.load_state_dict(best_state)
        vp, tp = predict_nn(model, vl), predict_nn(model, tel)
        del model, opt, crit, tl, vl, tel, best_state; gc.collect()
        if torch.cuda.is_available(): torch.cuda.empty_cache()
    ...
    def _run_pl():
        t0 = time.time(); oof = np.zeros(n_tr); test_pred = np.zeros(n_te); aucs = []
        base = CFG.SEED
        for sd in PL_SEEDS:
            CFG.SEED = sd; oof_s = np.zeros(n_tr)
            print(f'[pseudo] seed {sd}')
            for k, (tri, vai) in enumerate(folds):
                vp, tp, a = train_nn_pl(tri, vai)
                oof_s[vai] = vp; test_pred += tp / (len(folds) * len(PL_SEEDS)); aucs.append(a)
                gc.collect(); print(f'  fold {k}: AUC={a:.4f}')
            oof += oof_s / len(PL_SEEDS)
        CFG.SEED = base
        return dict(oof=oof, test=test_pred, fold_aucs=aucs, auc=roc_auc_score(yv, oof), kind='nn',
                    time=time.time() - t0)

    RESULTS['NN-gru-pseudo'] = load_or('nn_pseudo.joblib', _run_pl)
    r = RESULTS['NN-gru-pseudo']
    base_auc = RESULTS.get('NN-gru', {}).get('auc', 0)
    print(f"\nNN-gru-pseudo: OOF AUC={r['auc']:.5f}  (база NN-gru была {base_auc:.5f})  time={r['time']:.1f}s")
else:
    print('Псевдолейблинг пропущен: нет final_test или сеть отключена.')

if CFG.RUN_NN and HAS_TORCH:
    for _v in ('comb_flat', 'comb_off', 'comb_lab', 'pseudo', 'test_ids_comb'):
        globals().pop(_v, None)
    gc.collect()
    if 'X' not in globals():
        _f = joblib.load(apath('features.joblib')); X, X_test = _f['X'], _f['X_test']; del _f; gc.collect()
    ...
    n_tr, n_te = len(tr_off) - 1, len(te_off) - 1
    cols = list(X.columns); n_cols = len(cols)
    means = np.empty(n_cols, np.float32); stds = np.empty(n_cols, np.float32)
    tab_tr = np.empty((n_tr, n_cols), dtype=np.float32)
    for j, c in enumerate(cols):
        v = np.nan_to_num(X[c].to_numpy(dtype=np.float32), nan=0.0, posinf=0.0, neginf=0.0)
        m = float(v.mean()); s = float(v.std()); s = s if s > 1e-12 else 1.0
        means[j] = m; stds[j] = s
        tab_tr[:, j] = (v - m) / s
    del X; gc.collect()
    tab_te = np.empty((n_te, n_cols), dtype=np.float32)
    for j, c in enumerate(cols):
        v = np.nan_to_num(X_test[c].to_numpy(dtype=np.float32), nan=0.0, posinf=0.0, neginf=0.0)
        tab_te[:, j] = (v - means[j]) / stds[j]
    del X_test; gc.collect()
    assert len(tab_tr) == n_tr and len(tab_te) == n_te, 'размер таблицы != числу последовательностей'
    TAB_DIM = n_cols
    print(f'tab_tr: {tab_tr.shape} | tab_te: {tab_te.shape}')

    class SeqDatasetHybrid(Dataset):
        def __init__(self, flat, offsets, tab, idx=None, targets=None):
            self.flat = flat; self.offsets = offsets; self.tab = tab
            self.idx = np.arange(len(offsets) - 1) if idx is None else np.asarray(idx)
            self.targets = targets
        def __len__(self): return len(self.idx)
        def __getitem__(self, j):
            i = int(self.idx[j]); s, e = self.offsets[i], self.offsets[i + 1]
            seq = torch.from_numpy(np.ascontiguousarray(self.flat[s:e]))
            tb = torch.from_numpy(np.ascontiguousarray(self.tab[i]))
            y_ = -1.0 if self.targets is None else float(self.targets[i])
            return seq, tb, y_

    def collate_h(batch):
        seqs, tabs, ys = zip(*batch)
        lengths = torch.tensor([len(s) for s in seqs], dtype=torch.long)
        Lmax = int(lengths.max()); F = seqs[0].shape[1]
        padded = torch.zeros(len(seqs), Lmax, F, dtype=torch.long)
        for i, s in enumerate(seqs):
            padded[i, :len(s)] = s
        return padded, lengths, torch.stack(tabs), torch.tensor(ys, dtype=torch.float32)

    class CreditSeqNetHybrid(nn.Module):
        def __init__(self, vocab_sizes, tab_dim, hidden=128, n_layers=2, dropout=0.2, tab_hidden=256, tab_out=64):
            super().__init__()
            emb_dims = [min(16, max(2, int(round((v + 2) ** 0.5)))) for v in vocab_sizes]
            self.embs = nn.ModuleList([nn.Embedding(v + 2, d, padding_idx=0)
                                       for v, d in zip(vocab_sizes, emb_dims)])
            ...
            self.rnn = nn.GRU(in_dim, hidden, num_layers=n_layers, batch_first=True,
                              bidirectional=True, dropout=dropout if n_layers > 1 else 0.0)
            self.tab = nn.Sequential(nn.LayerNorm(tab_dim), nn.Linear(tab_dim, tab_hidden), nn.ReLU(),
                                     nn.Dropout(dropout), nn.Linear(tab_hidden, tab_out), nn.ReLU())
            head_in = hidden * 2 + tab_out
            self.head = nn.Sequential(nn.LayerNorm(head_in), nn.Linear(head_in, hidden),
                                      nn.ReLU(), nn.Dropout(dropout), nn.Linear(hidden, 1))
        def forward(self, x, lengths, tab):
            h = torch.cat([emb(x[:, :, i]) for i, emb in enumerate(self.embs)], dim=-1)
            packed = nn.utils.rnn.pack_padded_sequence(h, lengths.cpu(), batch_first=True,
                                                       enforce_sorted=False)
            _, h_n = self.rnn(packed)
    ...
    def predict_h(model, loader):
        model.eval(); out = []
        with torch.no_grad():
            for xb, lb, tb, *_ in loader:
                xb, lb, tb = xb.to(DEVICE), lb.to(DEVICE), tb.to(DEVICE)
                out.append(torch.sigmoid(model(xb, lb, tb)).cpu().numpy())
        return np.concatenate(out)

    def train_nn_h(tr_idx, va_idx):
        seed_everything()
        max_train = getattr(CFG, 'NN_MAX_TRAIN', 1_500_000)
        if len(tr_idx) > max_train:
            tr_idx = np.random.default_rng(CFG.SEED).choice(tr_idx, size=max_train, replace=False)
        yv = y.values; ytr = yv[tr_idx]; yva = yv[va_idx]
        pos_w = (len(ytr) - ytr.sum()) / max(ytr.sum(), 1)
        rank_w = getattr(CFG, 'NN_RANK_W', 1.0); nw = getattr(CFG, 'NN_WORKERS', 0)
        tl  = DataLoader(SeqDatasetHybrid(tr_flat, tr_off, tab_tr, idx=tr_idx, targets=yv),
                         batch_size=CFG.NN_BATCH, shuffle=True,  collate_fn=collate_h, num_workers=nw)
        vl  = DataLoader(SeqDatasetHybrid(tr_flat, tr_off, tab_tr, idx=va_idx, targets=yv),
                         batch_size=CFG.NN_BATCH, shuffle=False, collate_fn=collate_h, num_workers=nw)
        tel = DataLoader(SeqDatasetHybrid(te_flat, te_off, tab_te),
                         batch_size=CFG.NN_BATCH, shuffle=False, collate_fn=collate_h, num_workers=nw)
        model = CreditSeqNetHybrid(vocab_sizes, tab_dim=TAB_DIM, hidden=CFG.NN_HIDDEN).to(DEVICE)
        opt = torch.optim.AdamW(model.parameters(), lr=CFG.NN_LR, weight_decay=1e-5)
        crit = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([pos_w], device=DEVICE))
        best_auc, best_state = -1, None
        for ep in range(CFG.NN_EPOCHS):
            model.train()
            for xb, lb, tb, yb in tl:
                xb, lb, tb, yb = xb.to(DEVICE), lb.to(DEVICE), tb.to(DEVICE), yb.to(DEVICE)
                opt.zero_grad()
                logits = model(xb, lb, tb)
                loss = crit(logits, yb) + rank_w * pairwise_logistic_loss(logits, yb)
                loss.backward(); opt.step()
            auc = roc_auc_score(yva, predict_h(model, vl))
            if auc > best_auc:
                best_auc = auc; best_state = {k: v.cpu().clone() for k, v in model.state_dict().items()}
            print(f'    эпоха {ep+1}/{CFG.NN_EPOCHS}  val AUC={auc:.4f}')
        model.load_state_dict(best_state)
        vp, tp = predict_h(model, vl), predict_h(model, tel)
        del model, opt, crit, tl, vl, tel, best_state; gc.collect()
        if torch.cuda.is_available(): torch.cuda.empty_cache()
        return vp, tp, best_auc
    ...
    def _run_hybrid():
        ...
        for sd in HYB_SEEDS:
            CFG.SEED = sd; oof_s = np.zeros(n_tr)
            print(f'  [hybrid] seed {sd}')
            for k, (tri, vai) in enumerate(folds):
                vp, tp, a = train_nn_h(tri, vai)
                oof_s[vai] = vp; test_pred += tp / (len(folds) * len(HYB_SEEDS)); aucs.append(a)
                gc.collect(); print(f'  fold {k}: AUC={a:.4f}')
            oof += oof_s / len(HYB_SEEDS)
            print(f'  seed {sd}: OOF AUC={roc_auc_score(y.values, oof_s):.4f}')
        CFG.SEED = base
        return dict(oof=oof, test=test_pred, fold_aucs=aucs, auc=roc_auc_score(y.values, oof),
                    kind='nn', time=time.time() - t0)

    RESULTS['NN-gru-hybrid'] = load_or('nn_hybrid.joblib', _run_hybrid)
    r = RESULTS['NN-gru-hybrid']
    base_auc = RESULTS.get('NN-gru', {}).get('auc', 0)
    print(f"\nNN-gru-hybrid: OOF AUC={r['auc']:.5f}  (база NN-gru {base_auc:.5f})  time={r['time']:.1f}s")
else:
    print('Гибрид пропущен: сеть отключена.')
...
yv = y.values
models = [k for k, v in RESULTS.items() if 'oof' in v]
OOF  = np.column_stack([RESULTS[k]['oof']  for k in models])
TEST = np.column_stack([RESULTS[k]['test'] for k in models])
A = lambda p: roc_auc_score(yv, p)
print('Модели:', models)

def to_rank(M): return np.column_stack([rankdata(M[:, j]) / len(M) for j in range(M.shape[1])])
OOFr, TESTr = to_rank(OOF), to_rank(TEST)

cands = {}
p = cross_val_predict(LogisticRegression(C=1.0, max_iter=2000), OOF, yv, cv=5, method='predict_proba')[:, 1]
cands['logreg'] = (A(p), LogisticRegression(C=1.0, max_iter=2000).fit(OOF, yv).predict_proba(TEST)[:, 1])
pr = cross_val_predict(LogisticRegression(C=1.0, max_iter=2000), OOFr, yv, cv=5, method='predict_proba')[:, 1]
cands['logreg_rank'] = (A(pr), LogisticRegression(C=1.0, max_iter=2000).fit(OOFr, yv).predict_proba(TESTr)[:, 1])
cands['rank_avg'] = (A(OOFr.mean(1)), TESTr.mean(1))

def hill_cv(OOFr, TESTr, yv, iters=40):
    oof_pred = np.zeros(len(yv)); test_acc = np.zeros(len(TESTr))
    for tri, vai in KFold(5, shuffle=True, random_state=CFG.SEED).split(OOFr):
        idx = []; cur = np.zeros(len(tri)); yt = yv[tri]
        for _ in range(iters):
            bj, ba = -1, -1
            for j in range(OOFr.shape[1]):
                c = (cur * len(idx) + OOFr[tri, j]) / (len(idx) + 1)
                a = roc_auc_score(yt, c)
                if a > ba: ba, bj = a, j
            idx.append(bj); cur = (cur * (len(idx) - 1) + OOFr[tri, bj]) / len(idx)
        oof_pred[vai] = OOFr[vai][:, idx].mean(1); test_acc += TESTr[:, idx].mean(1) / 5
    return A(oof_pred), test_acc
cands['hill_rank'] = hill_cv(OOFr, TESTr, yv)

for k, (a, _) in sorted(cands.items(), key=lambda x: -x[1][0]):
    print(f'  {k:14s} OOF AUC={a:.5f}')
best = max(cands, key=lambda k: cands[k][0])
print('Лучшее:', best, '=', round(cands[best][0], 5))

final_test = cands[best][1]
sub = sample_sub[[CFG.ID_COL]].merge(
    pd.DataFrame({CFG.ID_COL: X_test.index.values, CFG.TARGET: final_test}), on=CFG.ID_COL, how='left')
sub[CFG.TARGET] = sub[CFG.TARGET].fillna(sub[CFG.TARGET].mean())
assert sub[CFG.ID_COL].is_unique and len(sub) == len(sample_sub)
sub.to_csv(CFG.SUB_OUT, index=False, float_format='%.6f')
print('Сабмит:', CFG.SUB_OUT, '| строк:', len(sub))
